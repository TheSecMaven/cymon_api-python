...
engine = create_engine('sqlite:///IP_Reportv2.db')   #Setup the Database
...
def send_request(apiurl, scanurl, headers,output):   #This function makes a request to the X-Force Exchange API using a specific URL and headers. 
    if(proxies == ""):
        response = requests.get(apiurl,'''params = {'ioc_type':'ipv4','ioc_value':'1.2.3.4','from':'2016-06-20'}''', headers=headers,timeout=20)
    else:
        response = requests.get(apiurl, proxies=proxies,headers=headers,timeout=20)
...
if __name__ == "__main__":
    ...
    data = base64.b64decode(jwt.split('.')[1] + "===")
...
if (options.s_ip is not "none"):    #If the -i option was used
    ...
    apiurl = url + "ioc/query/ipv4/" + scanurl + "?from=2016-06-20"
