...
if (options.s_category is not "none"):
    if(options.s_max is not "none"):
        ...
        apiurl = url + "/api/nexus/v1/blacklist/ip/" + category + "/?days=1" +  "&limit=" + scanurl 
        all_json = send_request(apiurl,category+scanurl,headers)
    else:
        days = int(options.days)    #If the --max was not specified it defers to 10
        scanurl = options.s_category
        apiurl = url + "/api/nexus/v1/blacklist/ip/" + scanurl + "/"
        all_json = send_request(apiurl,scanurl+str(days),headers)
